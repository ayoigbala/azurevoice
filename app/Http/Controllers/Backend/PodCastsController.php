    ...
    public function import()
    {
        ...
        if (isset($rss->channel)) {
            ...
            if ($rss->channel->children('itunes', true)) {
                ...
                if (isset($rss->channel->image->url) || isset($itunes->image->attributes()->href)) {
                    $imageUrl = isset($rss->channel->image->url) ? reset($rss->channel->image->url) : reset($itunes->image->attributes()->href);
                    $podcast->addMediaFromUrl($imageUrl)
                        ->usingFileName(time() . '.jpg')
                        ->toMediaCollection('artwork', config('settings.storage_artwork_location', 'public'));
                }
            }
